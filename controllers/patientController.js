...
module.exports.getMyBalances = async (req, res) => {
  const { id } = req.params;
  try {
    const balances = await Treatment.query()
      .select(
        'treatment.description',
        'treatment.date_treated',
        raw(
...
module.exports.cancelAppointment = async (req, res) => {
  ...
  try {
    const cancelAppointment = await Appointment.query().patchAndFetchById(
      appointmentId,
      { status: 'cancelled' }
    );
...
module.exports.validatePatientCode = async (req, res) => {
  ...
  try {
    ...
    if (!isExist) return res.send({ isValid: false });
...
module.exports.patients = async (req, res) => {
  ...
  if (!search)
    patients = patients = await Patient.query()
      .select(
        'patient.name',
        raw(
          `(SELECT date_treated FROM treatment WHERE treatment.patient_id = patient.id ORDER BY UNIX_TIMESTAMP(date_treated) DESC LIMIT 1) as last_visit`
...
module.exports.create = async (req, res) => {
  try {
    ...
    const alphabet =
      '0123456789abcdefghjklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_';
    ...
    do {
      ...
      patients.forEach((patient) => {
        if (newPatientCode === patient.code) isUnique = false;
      });
    } while (!isUnique);
    ...
    if (!patient)
      return res.status(500).send({ error: 'Internal server error' });
    const adultTeethChart = await AdultTeethChart.query().insert({
      id: patient.id,
    });
    const childTeethChart = await ChildTeethChart.query().insert({
      id: patient.id,
    });
...
module.exports.updateContactNumber = async (req, res) => {
  ...
  try {
    const result = await Patient.query().patchAndFetchById(patientId, {
      contact_number,
    });
    return res.sendStatus(200);
  } catch (err) {
...
module.exports.updateChildTeethChart = async (req, res) => {
  ...
  try {
    const result = await ChildTeethChart.query()
      .patch(newChart)
      .where('id', id);
    return res.sendStatus(200);
  } catch (err) {
...
module.exports.updateAdultTeethChart = async (req, res) => {
  ...
  const newChart = req.body;
  try {
    const result = await AdultTeethChart.query()
      .patch(newChart)
      .where('id', id);
    return res.sendStatus(200);
  } catch (err) {
