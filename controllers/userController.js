...
module.exports.validateUsername = async (req, res) => {
  ...
  try {
    ...
    if (isExist) return res.send({ isValid: false, username });
...
module.exports.validateEmail = async (req, res) => {
  ...
  try {
    ...
    if (isExist) return res.send({ isValid: false, email });
...
module.exports.users = async (req, res) => {
  ...
  let users;
  if (!search) users = await User.query().orderBy('id', 'desc');
  else {
    users = await User.query()
      .whereRaw(
        `LOWER(name) like "%${search.toLowerCase()}%" OR LOWER(username) like "%${search.toLowerCase()}%"`
      )
      .orderBy('id', 'desc');
  }
...
module.exports.register = async (req, res) => {
  ...
  try {
    ...
    const [patient] = await Patient.query()
      .select('id')
      .where('code', newUser.patient_code);
    if (!patient)
      return res.status(500).send({ error: 'Internal server error' });
  ...
  try {
    ...
    if (isPasswordCorrect) {
      ...
      res
        ...
        .status(200)
        .send({ user: payload });
    } else return res.send({ error: 'Invalid username or password' });
  } catch (err) {
    console.log(err);
    return res.status(500).send({ error: 'Internal server error!' });
  }
...
module.exports.resetPassword = async (req, res) => {
  ...
  try {
    ...
    const [user] = await User.query()
      .select('password')
      .where('emailaddress', decodeWithoutVerify.emailaddress);
    ...
    jwt.verify(token, user.password, async (err, decoded) => {
      if (err) {
        ...
        const saltRounds = 10;
        const hashedPassword = await bcrypt
          .hash(decoded.newPassword, saltRounds)
          .then((hash) => hash);
        await User.query()
...
module.exports.forgotPassword = async (req, res) => {
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        user: config.gmailUser, // REAL GMAIL EMAIL
        pass: config.gmailPassword, // REAL GMAIL PASSWORD
      },
    });
    ...
    const info = await transporter.sendMail({
      ...
      from: `Andres Dental Clinic <${config.gmailUser}>`,
